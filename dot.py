class DotBuilder:
    ...
    def get_dot(self, tree, node_styler_class):
        ...
        graph.append('edge [fontsize="11"]; \n')
        graph.append('node [fontsize="12" shape="box" style="filled"]; \n')
        
        stack = [tree.root_node]
        stack_pos = [-1, 0]
        
        total_time = tree.get_total_time()
        ...
        while len(stack):
            ...
            linewidth = 19.0 * stack[-1].sum_inclusive_time/total_time + 1
            if stack[-1].call_count == 1:
                node_label = '%s\\n%dms / %.1f%%' % (fn, stack[-1].sum_self_time/1000, 100. * stack[-1].sum_self_time/total_time)
                edge_label = '%dms / %.1f%%' % (stack[-1].sum_inclusive_time/1000, 100. * stack[-1].sum_inclusive_time/total_time)
            else:
                node_label = '%s\\n%dx\[%dms..%dms] = %dms / %.1f%%' % (fn, stack[-1].call_count, stack[-1].min_self_time/1000, stack[-1].max_self_time/1000, stack[-1].sum_self_time/1000, 100. * stack[-1].sum_self_time/total_time)
                edge_label = '%dx\[%dms..%dms] = %dms / %.1f%%' % (stack[-1].call_count, stack[-1].min_inclusive_time/1000, stack[-1].max_inclusive_time/1000, stack[-1].sum_inclusive_time/1000, 100. * stack[-1].sum_inclusive_time/total_time)
